...
// or a HCRYPTPROV depending on the flags given, and whether the caller is responsible for freeing it (CryptoAPI may
// instead have handed back a handle it still owns in its own cache).
func cryptAcquireCertificatePrivateKey(certContext uintptr, flags uint32) (provContext uintptr, callerMustFree bool, err error) {
	pvParameters := uint32(0)
	phCryptProvOrNCryptKey := uintptr(0)
	pdwKeySpec := uint32(0) // Can be 0, AT_KEYEXCHANGE, AT_SIGNATURE, or CERT_NCRYPT_KEY_SPEC
	// Win32 BOOL is a 4-byte int, not a Go bool (which is 1 byte) -- writing
	// the API's 4-byte output through a *bool would overrun adjacent stack
	// memory. int32 matches BOOL's actual size.
	var pfCallerFreeProvOrNCryptKey int32
	...
	if r0 == 0 {
		return 0, false, fmt.Errorf("r0 was 0")
	}
	...
	return phCryptProvOrNCryptKey, pfCallerFreeProvOrNCryptKey != 0, nil
}

// nCryptFreeObject releases a handle returned by cryptAcquireCertificatePrivateKey
// when the caller was told it owns that handle.
func nCryptFreeObject(hObject uintptr) error {
	r0, _, e1 := syscall.Syscall(procNCryptFreeObject.Addr(), 1, hObject, 0, 0)
	if r0 != 0 {
		return e1
	}
	return nil
...
func Sign(alg string, cert *Certificate, data []byte) (*pkcs7.PKCS7, error) {
	...
	if disablePINCache {
		...
		nCryptHandle, freeHandle, err = cryptAcquireCertificatePrivateKey(cert.certContext, acquireFlags)
		if err != nil {
			return nil, err
		}
	}
	...
	if disablePINCache && nCryptHandle != 0 {
		defer func() {
			...
			// Only free the handle when Windows told us we own it --
			// CRYPT_ACQUIRE_CACHE_FLAG can hand back a handle CryptoAPI's
			// own cache still holds, and freeing that one would be a
			// double-free of a handle we don't own.
			if freeHandle {
				if freeErr := nCryptFreeObject(nCryptHandle); freeErr != nil {
					fmt.Printf("Could not free NCrypt key handle: %v\n", freeErr)
				}
			}
		}()
	}
